...
const url = 'https://search.daum.net/search?w=news&cluster=y&q=금융+서비스&p=2'
...
function parseNews(newsElem) {
    ...
    const desc = newsElem.find('.c-item-contents .conts-desc').text();
...
// axios.get(`https://search.daum.net/search?nil_suggest=btn&w=news&DA=SBC&cluster=y&q=금융+서비스`).then(resp=>{
//     return resp.data;
// }).then(data => {
//     const $ = cheerio.load(data);

//     const companyTags = $('.c-list-basic .tit_item
